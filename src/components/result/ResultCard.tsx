...
import {getResultCardAsset} from "@/lib/resultCardAsset";
...
export function ResultCard({cell}: ResultCardProps) {
  ...
  const {path: resultCardImage} = getResultCardAsset(cell.id, locale);
  ...
  return (
    <section className="motion-reveal-pop mx-4 my-4 flex w-full max-w-[340px] flex-1 flex-col rounded-[28px] border border-white/85 bg-[linear-gradient(180deg,#ffffff_0%,#f5f8ff_100%)] px-4 pb-4 pt-5 shadow-[0_18px_36px_rgba(86,112,166,0.22)]">
      ...
      {!cardImageFailed ? (
        <div className="motion-reveal-up motion-stagger-1 mt-5 overflow-hidden rounded-[20px] border border-[#dbe6f8] bg-white shadow-[0_14px_28px_rgba(87,112,160,0.16)]">
          <Image
            src={resultCardImage}
            alt={`${cell.name} card`}
            width={1080}
